...
- [2020-12-28](#2020-12-28)
    ...
    - [列举常用的 BOM 对象和常用 window 对象方法](#列举常用的-bom-对象和常用-window-对象方法)
...
- [2020-12-25](#2020-12-25)
    ...
    - [Vue 中，`v-if` 和 `v-show` 指令的共同点和不同点](#vue-中-v-if-和-v-show-指令的共同点和不同点)
...
#### Vue 中，`v-if` 和 `v-show` 指令的共同点和不同点

- `v-if` 隐藏元素后，元素不存在 DOM 中。
- `v-show` 通过 `display: none` 隐藏元素。
...
- MVVM（Model-View-ViewModel）最早由微软提出来，它借鉴了桌面应用程序的 MVC（Model-View-Controller）思想。
- 在前端页面中，把 Model（数据模型）用纯 JavaScript 对象表示，View 负责显示，两者做到了最大限度的分离。
- 把 Model 和 View 关联起来的就是 ViewModel。
